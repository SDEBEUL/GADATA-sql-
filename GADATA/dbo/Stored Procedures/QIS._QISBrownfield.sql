...
-- =============================================
CREATE PROCEDURE [QIS.[QISBrownfield]
...
SET @QueryspotCheck1 = 'INSERT INTO #tempQisBrownfield SELECT convert(datetime,Getdate(),102) as timestamp,  Qischeck.timername,  Qischeck.program,  Qischeck.parameter,  Qischeck.[bos6000 value],  Qischeck.[required value],  Qischeck.remark
	FROM   (SELECT *  FROM   (SELECT tbl.timername,  tbl.program, tbl.text AS Parameter, tbl.value  AS [BOS6000 Value], QIS.QISLOG_GA2controlparameters.value AS [required Value], QIS.QISLOG_GA2controlparameters.minvalue, QIS.QISLOG_GA2controlparameters.maxvalue, CASE  WHEN tbl.value = QIS.QISLOG_GA2controlparameters.value OR tbl.value BETWEEN  QIS.QISLOG_GA2controlparameters.minvalue AND    QIS.QISLOG_GA2controlparameters.maxvalue THEN  ''Parameter OK''  ELSE ''Parameter NOK''   END  AS Remark                
	FROM   (' 
	SET @QueryspotCheck2 = 'AS derivedtbl_1) AS tbl INNER JOIN QIS.QISLOG_GA2controlparameters ON tbl.param_id = QIS.QISLOG_GA2controlparameters.parameter_id  WHERE  QIS.QISLOG_GA2controlparameters .generalparameter = ''0'' ) tbl )QisCheck '
...
SET 
@QueryMidairCheck1 = 
' INSERT INTO #tempQisBrownfield SELECT Getdate() AS Timestamp,  Qischeck.timername,  Qischeck.program,  Qischeck.parameter,  Qischeck.[bos6000 value],  Qischeck.[required value],  Qischeck.remark FROM   (SELECT * from (SELECT tbl.timername,                 tbl.program,                 tbl.text                           AS Parameter,                 tbl.value                          AS [BOS6000 Value],                 QIS.QISLOG_GA2controlparameters.value AS [required Value],                 QIS.QISLOG_GA2controlparameters.minvalue,                 QIS.QISLOG_GA2controlparameters.maxvalue,                 CASE                   WHEN tbl.value = QIS.QISLOG_GA2controlparameters.value THEN                   ''Parameter OK''                   ELSE ''Parameter NOK''                END                                AS remark         FROM   ('
SET @QueryMidairCheck2 = '  AS derivedtbl_1) AS tbl  INNER JOIN QIS.QISLOG_GA2controlparameters          ON tbl.param_id = QIS.QISLOG_GA2controlparameters.parameter_id   WHERE  QIS.QISLOG_GA2controlparameters.midairparameter = 1) tbl) Qischeck '
SET @SQL2 = 
...
SET @QueryGeneralCheck2 = ' AS derivedtbl_1) AS tbl INNER JOIN QIS.QISLOG_GA2controlparameters ON tbl.param_id = QIS.QISLOG_GA2controlparameters.parameter_id)tbl)QisCheck '
SET @SQL3 = 
