...
FROM  GADATA.NGAC.rt_alarm AS rt with (NOLOCK) 
LEFT OUTER JOIN GADATA.NGAC.L_category as lc with (NOLOCK)  on lc.id = rt.CategoryId


LEFT JOIN GADATA.NGAC.c_controller as c with (NOLOCK) on c.id = rt.controller_id
GO
