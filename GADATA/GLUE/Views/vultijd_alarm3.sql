...
CREATE VIEW [GLUE].[vultijd_alarm3]
AS
...
join glue.VULTIJDreferentie3 as V on V.Controller=P.Name
...
,P.Name as Teamstername
, cast(H.time as datetime) as Timestamp
, CONCAT('huidige vultijd=' , CAST(ROUND(H.gemiddelde_vultijd_ms/1000.0,5) AS DECIMAL(6,3)) , '   |   bovengrens vultijd=' , V.UCLx)   as logtekst
, g.Product as Product
, g.Doser_type as Doser_type
, T.Vyear AS 'Year'
, T.Vweek AS 'Week'
, T.Vday AS 'Day'
, 'UCLx' as 'aantal keren standaardafwijking'
...
where/* T.Vday <> 'NULL' AND*/ T.Vday <> '6' AND V.UCLx < CAST(ROUND(H.gemiddelde_vultijd_ms/1000.0,5) AS DECIMAL(6,3))