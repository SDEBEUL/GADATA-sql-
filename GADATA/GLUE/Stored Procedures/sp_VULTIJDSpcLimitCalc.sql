...
CREATE PROCEDURE [GLUE].[sp_VULTIJDSpcLimitCalc]
...
vgm.Controller
,ref.*
FROM GADATA.GLUE.vultijd_gemiddelde_mediaan3 as vgm
left join GLUE.VULTIJDreferentie as ref on
ref.Controller = vgm.Controller
where  
...
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(1*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL'
,ROUND(AVG(vgm.gemiddelde_vultijd_s)-(1*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'LCL'
FROM GADATA.GLUE.vultijd_gemiddelde_mediaan3 as vgm
where 
vgm.Controller LIKE @Teamstername
AND
