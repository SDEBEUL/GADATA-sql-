...
CREATE PROCEDURE [GLUE].[sp_VULTIJDSpcLimitCalc4]
...
SELECT 
*
FROM GADATA.GLUE.VULTIJDreferentie2 as vgm
where  
...
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(1*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL1'
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(2*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL2'
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(3*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL3'
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(4*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL4'
,ROUND(AVG(vgm.gemiddelde_vultijd_s)+(5*Stdev(vgm.gemiddelde_vultijd_s)),2) as 'UCL5'
...
DELETE GLUE.VULTIJDreferentie2 FROM GLUE.VULTIJDreferentie2 as ref 
Where ref.Controller LIKE @Teamstername-- AND ref.tool_id LIKE @Toolmask
---------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
--Print 'calculate new limits'
---------------------------------------------------------------------------------------
INSERT INTO GLUE.VULTIJDreferentie2
select 
 vgm.Controller as 'Controller_id'
,Min(vgm.time) as 'SampleStart'
,Max(vgm.time) as 'SampleEnd'
